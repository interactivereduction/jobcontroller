...
class JobCreatorTest(unittest.TestCase):
    ...
    def test_setup_archive_pv(self, client):
        ...
        client.V1CSIPersistentVolumeSource.assert_called_once_with(
            driver="smb.csi.k8s.io", read_only=True, volume_handle=pv_name,
    ...
    def test_setup_ceph_pv(self, client):
        ...
        self.assertEqual(_setup_ceph_pv(job_name, ceph_creds_k8s_secret_name, ceph_creds_k8s_namespace, cluster_id,
                                        fs_name, ceph_mount_path), pv_name)
        ...
        client.V1PersistentVolumeSpec.assert_called_once_with(
            capacity={'storage': '1000Gi'}, storage_class_name='', access_modes=['ReadWriteMany'],
            persistent_volume_reclaim_policy='Retain', volume_mode='Filesystem',
            csi=client.V1CSIPersistentVolumeSource.return_value
        )
        client.V1CSIPersistentVolumeSource.assert_called_once_with(
    ...
    @mock.patch('jobcreator.job_creator.load_kubernetes_config')
    @mock.patch('jobcreator.job_creator.client')
    def test_jobcreator_spawn_job_dev_mode_false(self, client, _, setup_ceph_pvc, setup_ceph_pv, setup_archive_pvc,
                                                 setup_archive_pv):
        ...
        job_creator.spawn_job(job_name, script, job_namespace, ceph_creds_k8s_secret_name, ceph_creds_k8s_namespace,
                              cluster_id, fs_name, ceph_mount_path, reduction_id, max_time_to_complete_job, db_ip,
                              db_username, db_password, runner_sha)

        self.assertEqual(client.BatchV1Api.return_value.create_namespaced_job.call_args.kwargs['namespace'],
                         job_namespace)
        self.assertEqual(client.BatchV1Api.return_value.create_namespaced_job.call_args.kwargs['body'],
                         client.V1Job.return_value)
        client.V1Job.assert_called_once_with(api_version="batch/v1",
                                             kind="Job",
                                             metadata=client.V1ObjectMeta.return_value,
                                             spec=client.V1JobSpec.return_value)
        client.V1ObjectMeta.assert_called_once_with(name=job_name,
                                                    annotations={
                                                        "reduction-id": str(reduction_id),
                                                        "pvs": str([setup_archive_pv.return_value,
                                                                    setup_ceph_pv.return_value]),
                                                        "pvcs": str([setup_archive_pvc.return_value,
                                                                     setup_ceph_pvc.return_value]),
                                                        "kubectl.kubernetes.io/default-container":
                                                            client.V1Container.return_value.name
                                                    })
        ...
        self.assertIn(call(name="ceph-mount",
                           persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource.return_value),
                      client.V1Volume.call_args_list)
        self.assertIn(call(claim_name=f"{job_name}-ceph-pvc", read_only=False),
                           client.V1PersistentVolumeClaimVolumeSource.call_args_list)
        client.V1Volume.assert_called_with(
            name="archive-mount",
            persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource.return_value)
        client.V1PersistentVolumeClaimVolumeSource.assert_called_with(claim_name=f"{job_name}-archive-pvc", read_only=True)
        ...
        setup_ceph_pv.assert_called_once_with(job_name, ceph_creds_k8s_secret_name,
                                              ceph_creds_k8s_namespace, cluster_id, fs_name, ceph_mount_path)
    ...
    @mock.patch('jobcreator.job_creator.load_kubernetes_config')
    @mock.patch('jobcreator.job_creator.client')
    def test_jobcreator_spawn_job_dev_mode_true(self, client, _, setup_ceph_pvc, setup_ceph_pv, setup_archive_pvc,
                                                 setup_archive_pv):
        job_name = mock.MagicMock()
        script = mock.MagicMock()
        job_namespace = mock.MagicMock()
